...
1. **Planning Agent** (`backend/planner.py`) — single-shot Claude call (`claude-sonnet-4-6`) that produces a JSON execution plan and saves it to the `run_plan` DB table. ~1-2K tokens per call.
